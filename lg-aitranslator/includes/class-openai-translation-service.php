...
class LG_OpenAI_Translation_Service extends LG_Abstract_Translation_Service {
    ...
    protected function call_translation_api($text, $source_lang, $target_lang) {
        ...
        if ($code !== 200) {
            ...
            throw new Exception(sprintf(__('OpenAI API error (code %1$d): %2$s', 'lg-aitranslator'), $code, $error_body)); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
        }

        $result = json_decode(wp_remote_retrieve_body($response), true);

        if (empty($result['choices'][0]['message']['content'])) {
            throw new Exception(__('Invalid response from OpenAI API', 'lg-aitranslator')); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
        }
